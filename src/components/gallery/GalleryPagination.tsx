...
export function GalleryPagination({
  ...
  return (
    ...
    >
      {page > 1 ? (
        <Link href={buildHref(page - 1, tagFilter, characterSlug)} className={linkCls} aria-label="Previous page">
      ...
      <div className="flex items-center gap-1">
        {pages.map((p, i) =>
          p === 'ellipsis' ? (
            ...
            <Link key={p} href={buildHref(p, tagFilter, characterSlug)} className={linkCls}>
              {p}
            </Link>
          )
        )}
      ...
      {page < totalPages ? (
        <Link href={buildHref(page + 1, tagFilter, characterSlug)} className={linkCls} aria-label="Next page">
          Next
