-- Fuzzy admin gallery search: subsequence match on normalized text + linked character names.
...
AS $$
  ...
  WHERE trim(coalesce(p_search, '')) = ''
     ...
     OR public.gallery_text_fuzzy_match(gi.name, p_search)
     OR public.gallery_text_fuzzy_match(gi.drive_file_id, p_search)
     ...
     OR EXISTS (
       ...
       WHERE t.tag ILIKE '%' || p_search || '%'
          OR public.gallery_text_fuzzy_match(t.tag, p_search)
     );
$$;

REVOKE ALL ON FUNCTION public.gallery_text_fuzzy_match(text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_gallery_search_ids(text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.gallery_text_fuzzy_match(text, text) TO postgres;
GRANT EXECUTE ON FUNCTION public.gallery_text_fuzzy_match(text, text) TO service_role;

GRANT EXECUTE ON FUNCTION public.admin_gallery_search_ids(text) TO postgres;
GRANT EXECUTE ON FUNCTION public.admin_gallery_search_ids(text) TO service_role;
